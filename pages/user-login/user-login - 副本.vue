...
<script>
	...
	export default {
		...
		onLoad: function(options) {
			...
			// 判断是否处于微信环境，如果是微信环境就执行获取code并登录的方法，否则就跳转到h5的登录页面
		...
		methods: {
			...
			// 获取code并登录   
			getWXCode() {
				let appid = 'wxa2a6315d32072c7e'; //公众号的唯一标识
				let code = this.getUrlCode().code; //是否存在code
				let url =  'http://h5.hnliyue.cn:8088/#/pages/user-login/user-login';//授权后重定向的回调链接地址
				if (code == null || code === "") {
